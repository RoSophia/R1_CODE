...
object color_detection {
    ...
    fun iscolorRed(colorArray: DoubleArray): Boolean{
        ...
        return angdiff(HSV[0], redHue) < redMaxHueVariation && HSV[1] > redMinSaturation && HSV[2] > redMinValue
    }

    fun iscolorBlue(colorArray: DoubleArray): Boolean{
        val HSV = RGBtoHSV(colorArray)
        return angdiff(HSV[0], blueHue) < blueMaxHueVariation && HSV[1] > blueMinSaturation && HSV[2] > blueMinValue
    }
